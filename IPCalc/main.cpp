	...
	case WM_COMMAND:
	{
		...
		case IDC_EDIT_PREFIX:
		{
			if (HIWORD(wParam) == EN_CHANGE)
			{
				...
				INT iPrefix = atoi(szIPprefix);	//atoi() - ANSI/ASCII string to INT. ANSI/ASCII-string (NULL-Terminal Line) - строка заканчивающаяся нулём. Массив элементов 'CHAR', последним элементом которого является '\0'
				if (iPrefix > 32)
				{
					SendMessage(hEditPrefix, WM_SETTEXT, 0, (LPARAM)"32");
					iPrefix = 32;
				}
				dwIPmask = UINT_MAX;	//Все биты в Маске устанавливаются в 1
